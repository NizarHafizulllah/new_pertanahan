<?php
class baru_verifikasi extends master_controller {
	function baru_verifikasi(){
		parent::__construct();
	...
	function index(){
		...
		$content = $this->load->view("baru_verifikasi_view",$data_array,true);
...
function get_data(){
     ...
     <ul class=\"dropdown-menu\">
		<li><a  href=\" " . site_url("baru_verifikasi/detail/".$daft_id) ."\" ><span class=\"glyphicon glyphicon-chevron-right\"></span> Detail </a></li>
		<li><a  target=blank  href=\" " . site_url("baru_verifikasi/cetak_permohonan/".$daft_id) ."\" ><span class=\"glyphicon glyphicon-print\"></span> Cetak Permohonan </a></li>
		<li><a  target=blank  href=\" " . site_url("baru_verifikasi/cetak_berkas/".$daft_id) ."\" ><span class=\"glyphicon glyphicon-print\"></span> Cetak Surat Blokir </a></li>
	".	
       $link
	."</ul>
...
function detail($daft_id){
	...
	// show_array($data_array);
		...
		$content = $this->load->view("baru_verifikasi_view_detail",$data_array,true);
...
if($ret_service['error'] == true) { // koneksi ke server polda gagal.
			$ret_arr['error']  = true;
			$ret_arr['message'] = "KONEKSI TERPUTUS DENGAN SERVER PUSAT";
}

else { // koneksi ke server polda berhasil 
			if($ret_service['data']['Result'] == false  ) {  // result true 
				    //  and  $ret_service['data']['KodeError'] <> 0
					$ret_arr['error']  = true;
					...
					$ret_arr['message'] =  $error[$error_code];
			...
			$res = $this->db->update("t_pendaftaran",$data);
				if($res){
					...
					$ret_arr['error']  = true;
					$ret_arr['message'] = "GAGAL BLOKIR ".mysql_error();
				}
