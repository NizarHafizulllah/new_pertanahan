<?php
class admin_lap_model extends CI_Model {
	function admin_lap_model(){
		parent::__construct();
	}
...
function get_pendaftaran_detail($vleasing_id,$daft_id) {
   ...
   if((p.approved=0 and approved_error = 4),'BPKB SUDAH TERBIT DAN SUDAH TIDAK AKTIF',
   if((p.approved=0 and approved_error = 5),'USER TIDAK DITEMUKAN',
   IF(p.approved=1, 'BLOKIR BPKB', IF(p.approved=2, 'CEK ABSAH POLISI, NOMOR RANGKA TIDAK DITEMUKAN','-' ))))) ) ) ) AS approved,
