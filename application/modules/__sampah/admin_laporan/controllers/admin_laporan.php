<?php
class admin_laporan extends master_controller {
	function admin_laporan(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("admin_lap_model","dm");
	...
	function index(){
		...
		$data_array['arr_jenis_permohonan'] = $this->dm->arr_jenis_permohonan;	 
		...
		$this->set_subtitle("LAPORAN  PENDAFTARAN BLOKIR KENDARAAN   ");
		$this->set_title("LAPORAN  PENDAFTARAN BLOKIR KENDARAAN  ");
...
function get_data(){
        ...
        $leasing_id = $_REQUEST['columns'][3]['search']['value'];
        $id_polda = $_REQUEST['columns'][2]['search']['value'];
        $jenis_permohonan = $_REQUEST['columns'][1]['search']['value'];
      ...
      //  order[0][column]
		...
		$daft_id = $row['daft_id'];
        	...
        	$link = ($row['jenis_permohonan']=="L")?"<li><a  target=blank  href=\" " . site_url("polisi_verifikasi/cetak_keabsahan/".$daft_id) ."\" ><span class=\"glyphicon glyphicon-print\"></span> Cetak Dokumen Keabsahan </a></li>":"";
     ...
     <ul class=\"dropdown-menu\">
		<li><a  href=\" " . site_url("$this->controller/detail/".$daft_id) ."\" ><span class=\"glyphicon glyphicon-chevron-right\"></span> Detail </a></li>
		<li><a  target=blank  href=\" " . site_url("$this->controller/cetak_permohonan/".$daft_id) ."\" ><span class=\"glyphicon glyphicon-print\"></span> Cetak Permohonan </a></li>
		<li><a  target=blank  href=\" " . site_url("$this->controller/cetak_berkas/".$daft_id) ."\" ><span class=\"glyphicon glyphicon-print\"></span> Cetak Surat Blokir </a></li>
	".	
       $link
	."</ul>
	...
	if($userdata['user_level']==3){
		...
		if($data_daft->status <> '2' ) {
			...
			if($this->pilihan=="B") {
				...
				if($ret_service['error']==false)
				{
						if($ret_service['data']['Result'] == true and $ret_service['data']['KodeError'] == 0) {  // return dari server true
							$arr_data=array("daft_level3_date"=>date("Y-m-d"),
										"daft_level3_by"=>$userdata['user_id'],
										"status" => 3,
										"approved" => 1,
										"id_approval" => $ret_service['data']['IdApproval'],
										"no_blokir" => $ret_service['data']['NoRegBlokir'],
...
if($ret_service['error'] == true) {
...
else if($ret_service['data']['Result'] == true and  $ret_service['data']['KodeError'] <> 0  ) {
			$ret_arr['error']  = true;

			$error_code = $ret_service['data']['KodeError'];
	...
	$data['no_blokir'] = $ret_service['data']['BlokirData']->NoRegBlokir;
	$data['tgl_blokir'] = $this->tanggal($ret_service['data']['BlokirData']->TglBlokir);
	$data['tgl_blokir2'] = $this->tanggal($ret_service['data']['BlokirData']->TglAkhirBerlaku);
	$data['approved'] = 1;
...
function get_data_polda(){
	$data = $this->input->post();
	$this->db->select('pol.*')->
	from('m_polda pol')->join("polda_leasing pl","pol.id_polda=pl.id_polda");
	$this->db->where("pl.leasing_id",$data['leasing_id']);
	$res = $this->db->get();
	$html ="<option value='x'> == SEMUA POLDA ==  </option>";
	foreach($res->result() as $row) : 
		$html .="<option value=$row->id_polda>$row->nama_polda </option>";
	endforeach;
	echo $html;
}
